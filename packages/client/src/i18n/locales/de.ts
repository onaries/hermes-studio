...
jobTriggered: 'Job ausgelost',
  ...
  // Anderungsprotokoll
  changelog: {
    new_0_6_11_1: 'LAN device discovery umfasst jetzt pairing approvals, request history, duplicate request handling, status sync und cleanup beim Entzug von device access',
    new_0_6_11_2: 'LAN peer access ist ueber MCP verfuegbar, mit bundled MCP startup, command shims, reconnecting peer sockets, local connection listing und begrenzten terminal sessions',
    new_0_6_11_3: 'Unified backend TTS synthesis und MiMo voice cloning settings sind jetzt verfuegbar, inklusive playback vor dem Speichern',
    new_0_6_11_4: 'Gateway autostart settings zeigen profile filtering mit multi-select controls, whitelist behavior und Tests fuer startup profile selection',
    new_0_6_11_5: 'Chat scrolling und motion sind stabiler durch bottom-follow tuning, initial bottom scroll fixes, scroll lock updates und reduced-motion fade handling',
    new_0_6_11_6: 'Agent Bridge und runtime flows sind zuverlaessiger mit abort timeout handling, resume startup guards, execute-code approval memory, Windows gateway recovery und dev terminal proxy fixes',
    new_0_6_11_7: 'Security ist haerter durch Web UI origin policy, security headers und server-token loopback access guards',
    new_0_6_11_8: 'Group chat und tool output rendering sind stabiler mit room rejoin nach reconnect, erhaltenen reasoning/tool boundaries, sichererem tool payload rendering und besserer diff display',
    new_0_6_11_9: 'Skills und Workspace flows unterstuetzen jetzt external skill directories, import/delete refresh, source-path grouping, lokalisierte folder picker text und mobile skill action icons',
    new_0_6_11_10: 'Device layout, session titles, browser tab titles, Kanban parent task IDs und OpenRouter attribution text wurden verbessert',
