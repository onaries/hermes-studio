export default {
  ...
  chat: {
    ...
    clarifySubmit: '返信',
    clarifyDismiss: '閉じる',
    browserNotificationsEnable: 'ブラウザー通知を許可',
    browserNotificationsUnsupported: 'この環境ではブラウザー通知はサポートされていません。',
    browserNotificationsEnabled: 'ブラウザー通知を有効にしました。',
    approvalNotificationTitle: 'Hermes の承認が必要です',
    approvalNotificationBody: 'ツール実行の承認待ちです。',
    clarifyNotificationTitle: 'Hermes が回答を待っています',
    clarifyNotificationBody: 'エージェントがあなたの返信を待っています。',
    completionNotificationTitle: 'Hermes が完了しました',
    completionNotificationBody: '{title} が完了しました。',
    completionFailedNotificationTitle: 'Hermes タスクが失敗しました',
    completionFailedNotificationBody: '{title} はエラーで終了しました。',
    clarifyExpiresIn: '残り {time}',
    clarifyExpired: '期限切れ',
  ...
  settings: {
    ...
    display: {
      ...
      showToolMascotDesktop: 'デスクトップのツールマスコットを表示',
      showToolMascotDesktopHint: 'デスクトップでライブツール呼び出しの横にキャラクターを表示します',
      showToolMascotMobile: 'モバイルのツールマスコットを表示',
      ...
      browserNotifyOnComplete: '完了時のブラウザー通知',
      browserNotifyOnCompleteHint: 'チャット実行が完了したらブラウザー/デスクトップ通知を表示',
      browserNotifyOnCompleteDenied: 'ブラウザー通知がブロックされています。ブラウザーまたは OS 設定で許可してください。',
      ...
      terminalFontSizeHint: 'ターミナル文字サイズを調整します',
      terminalFontFamily: 'ターミナルのフォント',
      terminalFontFamilyHint: 'ターミナル文字に使う CSS font-family。空にすると既定値を使います',
  ...
  githubPreview: {
    ...
    activeVersion: "適用中のバージョン",
    activePath: "適用済みパス",
    activeUpdatedAt: "適用日時",
    currentTag: "現在の Tag",
    repoReady: "リポジトリ準備済み",
    ...
    applySuccess: "内部更新をインストールしました。再起動します",
  ...
  terminal: {
    ...
    mobileShortcutBar: 'モバイル端末ショートカットキー',
    hideMobileShortcutBar: 'モバイル端末ショートカットキーを隠す',
    showMobileShortcutBar: 'モバイル端末ショートカットキーを表示',
  ...
  // 更新履歴
  changelog: {
    new_0_6_14_1: 'Web サイトとドキュメントを更新し、ランディングページ再設計、ライトテーマ固定表示、新しい Hermes Studio クライアントマニュアルを追加しました',
    new_0_6_14_2: '完了通知により、デスクトップシェルと対応ブラウザで実行完了を知らせられるようになりました',
    new_0_6_14_3: 'デスクトップにカスタムタイトルバー、より整理された起動スプラッシュ、広い承認ドロワー、中央配置のロード状態を追加し、Linux のウィンドウ操作ボタンを修正しました',
    new_0_6_14_4: 'Workspace フォルダーに直接操作を追加し、UI からファイルやディレクトリを開く、管理する、利用する操作を簡単にしました',
    new_0_6_14_5: 'Profile と runtime 起動を安定化し、予約済み名前への rename 拒否、Linux deb 起動パス修正、旧 gateway cleanup 削除、Coding Agent 再開時の不要な bridge チェック削減を行いました',
    new_0_6_14_6: 'ブラウザを変えてもグループチャットの身份がより安定し、agent とメンバー名がアカウント/Profile 状態と揃うようになりました',
    new_0_6_14_7: 'パッケージ済み Web UI ファイルの圧縮処理を修正し、静的アセット配信をより軽く安全にしました',
    new_0_6_14_8: 'デバイスペアリングは起動ごとのペアリングコード、承認前のリクエスト検証、リクエスト制限により安全になり、信頼済みローカルフロー向けの LAN 検出は維持されます',
    new_0_6_14_9: 'デスクトップの Claude Code 検出はユーザー shell の PATH と一般的なパッケージマネージャーの場所を読むようになり、macOS と Linux でインストール済み coding agent を見つけやすくなりました',
    new_0_6_14_10: 'Hermes write gate は memory と skill の書き込み、保留承認レビュー、bridge モジュール分割、ローカライズされた承認 UI、改善されたデスクトップ起動をカバーします',
