...
async function openItem(item: SearchItem) {
  ...
  if (!chatStore.sessions.some(session => session.id === item.id) && typeof chatStore.addOrUpdateSession === 'function') {
    chatStore.addOrUpdateSession({
      ...
      agentNativeSessionId: item.agent_native_session_id,
      codingAgentId: item.agent === 'codex' ? 'codex' : item.agent === 'claude' ? 'claude-code' : undefined,
      codingAgentMode: item.source === 'coding_agent' || item.agent === 'codex' || item.agent === 'claude'
        ? (item.agent_mode === 'global' || item.agent_mode === 'scoped' ? item.agent_mode : item.provider === 'global' ? 'global' : 'scoped')
        : undefined,
      messages: [],
