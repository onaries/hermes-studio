...
function extractMessageArtifacts(message: Message): ArtifactFileReference[] {
  const content = message.content || '';
  if (!content.trim() || message.role === 'tool') return [];
...
const activeChatArtifactFiles = computed<ArtifactFileReference[]>(() => {
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    for (const file of extractMessageArtifacts(messages[i])) {
      addArtifactReference(refs, seen, file.path, file.name);
    }
  }
  return refs;
});
